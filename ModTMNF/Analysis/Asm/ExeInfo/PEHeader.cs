...
namespace ModTMNF.Analysis.Asm
{
    ...
    public class PeHeaderReader
    {
        ...
        public struct IMAGE_EXPORT_DIRECTORY
        {
            ...
            public UInt32 AddressOfOrdinals;    // RVA from base of image
        }
        ...
        private int numSectionsOffset;        
        private int sectionsOffset;
        private int importTableOffset;
        private int sizeOfImageOffset;
        ...
        public PeHeaderReader(string filePath)
        {
            ...
            using (FileStream stream = new FileStream(filePath, System.IO.FileMode.Open, System.IO.FileAccess.Read))
            {
                ...
                numSectionsOffset = (int)(reader.BaseStream.Position + 2);
                ...
                importTableOffset = (int)(stream.Position - (Marshal.SizeOf(typeof(IMAGE_DATA_DIRECTORY)) * 15));
                
                sizeOfImageOffset = importTableOffset;
                sizeOfImageOffset -= Marshal.SizeOf(typeof(IMAGE_DATA_DIRECTORY));// The last data directory
                sizeOfImageOffset -= Is32BitHeader ? 
                    (6 * 4) + (2 * 2) + (3 * 4) : 
                    (2 * 4) + (4 * 8) + (2 * 2) + (3 * 4);

                sectionsOffset = (int)stream.Position;
                ...
                headerLength = (int)stream.Position;
            }
        }

        public static PeHeaderReader FromFileEx(string file, out uint imageBase, out uint imageSize, out uint codeOffset, out uint codeSize)
        {
            PeHeaderReader peReader = new PeHeaderReader(file);
            imageBase = (uint)peReader.ImageBase;
            imageSize = peReader.SizeOfImage;

            codeOffset = 0;
            codeSize = 0;
            if (peReader.BaseOfCode > 0)
            {
                uint offset = codeOffset = peReader.BaseOfCode;
                codeSize = peReader.ImageSectionHeaders.FirstOrDefault(x => x.VirtualAddress == offset).VirtualSize;
                //codeSize = peReader.OptionalHeader32.SizeOfCode;// Seems less than the actual code section size, look into
            }
            else
            {
                // Assume first section is code section
                codeOffset = peReader.ImageSectionHeaders[0].VirtualAddress;
                codeSize = peReader.ImageSectionHeaders[0].VirtualSize;
            }

            return peReader;
        }

        public static bool IsFile32Bit(string filePath)
        {
        ...
        public int NumSectionsOffset
        {
            get { return numSectionsOffset; }
        }
        ...
        {
            get { return Is32BitHeader ? OptionalHeader32.BaseOfData : 0; }
        }
        ...
        {
            get { return Is32BitHeader ? OptionalHeader32.SizeOfImage : OptionalHeader64.SizeOfImage; }
        }

        public uint SizeOfHeader
        {
            get { return Is32BitHeader ? OptionalHeader32.SizeOfHeaders : OptionalHeader64.SizeOfHeaders; }
        }
        ...
        public IMAGE_DATA_DIRECTORY ImportTable
        {
            get { return Is32BitHeader ? OptionalHeader32.ImportTable : OptionalHeader64.ImportTable; }
        }

        public uint SectionAlignment
        {
            get { return Is32BitHeader ? OptionalHeader32.SectionAlignment : OptionalHeader64.SectionAlignment; }
        }
